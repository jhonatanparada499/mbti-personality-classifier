...
Notes: Paper seems to be a pipeline design to process either text or audio data(bimodal) using Whisper(ASR), a tokenizer, and then pass it to a "tuned" classification model called XLMRobertaClassifier. Their dataset are MBTI categories of profiles from forum posts(video links and text concatenated separated by ||)
